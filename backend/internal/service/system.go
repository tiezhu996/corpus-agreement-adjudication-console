...
func (service *SystemService) Login(request dto.LoginRequest) (dto.LoginResponse, error) {
	...
	claims := Claims{
		...
		RegisteredClaims: jwt.RegisteredClaims{
			Subject: fmt.Sprintf("%d", user.ID), Issuer: "corpus-agreement-api",
...
func (service *SystemService) ParseToken(encoded string) (dto.Actor, error) {
	...
	}, jwt.WithIssuer("corpus-agreement-api"), jwt.WithExpirationRequired())
...
func encodeSummary(value any) string {
	...
	sanitized, err := json.Marshal(redactAuditValue(decoded))
	if err != nil {
		return `{"summary":"unavailable"}`
	}
...
func decodeSummary(encoded string) map[string]interface{} {
	...
	if err := json.Unmarshal([]byte(encoded), &result); err != nil {
		return map[string]interface{}{"summary": "unavailable"}
	}
	return result
}
...
func sensitiveAuditKey(key string) bool {
	...
	switch compact {
	case "labels", "labelsjson", "finallabels", "examples", "examplesjson", "maskedtext",
		"password", "passwordhash", "token", "accesstoken", "refreshtoken", "authorization",
		"secret", "jwtsecret", "credential", "credentials", "cookie", "setcookie", "apikey", "privatekey":
		return true
	}
	return strings.Contains(compact, "rawtext") || strings.Contains(compact, "originalcontent") ||
		strings.HasSuffix(compact, "password") || strings.HasSuffix(compact, "token") ||
		strings.HasSuffix(compact, "secret") || strings.HasSuffix(compact, "credential")
}
